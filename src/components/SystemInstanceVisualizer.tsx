...
export const SystemInstanceVisualizer: FC<SystemVisualizerProps> = ({
  ...
  const buildComponentNodes = useCallback(() => {
    ...
    systemData.components.forEach((component, cidx) => {
      component.instances.forEach((instance, iidx) => {
        newNodes.push({
          id: instance.id,
          type: "default",
          data: {
            label: instance.id,
            hasChildren: instance?.childInstanceIds?.length ?? 0 > 0,
            sourceHandles:
              handleConnections.get(instance.id)?.sourceHandles ?? [],
            targetHandles:
              handleConnections.get(instance.id)?.targetHandles ?? [],
          },
          position: {
            x: 0,
            y: 0,
          },
          draggable: true, // instance.parentInstanceId ? false : true,
